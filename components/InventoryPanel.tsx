...
const RARITY_ORDER: Record<Item['rarity'], number> = {
  ...
  'Truyền Kỳ': 3,
  'Thần Thoại': 4,
  // FIX: Added 'Tối Thượng' to match the ItemRarity type.
  'Tối Thượng': 5,
};
...
const InventoryPanel: React.FC<InventoryPanelProps> = ({ player, onItemUse, onEquip }) => {
  ...
  const processedInventory = useMemo(() => {
    ...
    const uniqueEquippableItems: Item[] = [];

    player.inventory.forEach(item => {
      // If item has a slot (is equippable), treat it as unique and don't stack
      if (item.slot) {
        uniqueEquippableItems.push(item);
      } else { // Otherwise, stack by ID
    ...
    const uniqueAsStacked = uniqueEquippableItems.map(item => ({ item, count: 1 }));
    let allItems: StackedItem[] = [...uniqueAsStacked, ...stackedItems];


    // 1. Filter by category
    allItems = allItems.filter(stackedItem => {
    ...
    allItems.sort((a, b) => {
        switch (sortOrder) {
            ...
            default:
                 // Default sort: Rarity DESC, then Upgrade Level DESC, then Name ASC
                const rarityDiff = RARITY_ORDER[b.item.rarity] - RARITY_ORDER[a.item.rarity];
                if (rarityDiff !== 0) return rarityDiff;
                const upgradeDiff = (b.item.upgradeLevel || 0) - (b.item.upgradeLevel || 0);
    ...
    return allItems;
  }, [player.inventory, filter, searchQuery, sortOrder]);
  
  const selectedItemStack = useMemo(() => {
      if (!selectedItem) return null;
      return processedInventory.find(si => si.item === selectedItem);
  }, [selectedItem, processedInventory]);
  ...
  React.useEffect(() => {
    if (selectedItem && !processedInventory.some(si => si.item === selectedItem)) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-wrap gap-2 mb-2 flex-shrink-0">
          {(['Tất cả', 'Trang bị', 'Đan Dược', 'Tiêu hao', 'Sách Kỹ Năng', 'Nguyên liệu', 'Đan Lô'] as FilterType[]).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
        ...
        <div className="flex-grow space-y-2 overflow-y-auto pr-2 border-r border-gray-700">
          {processedInventory.length > 0 ? (
            processedInventory.map(({ item, count }, index) => (
              <button
                key={item.id + (item.upgradeLevel || '') + index}
                onClick={() => setSelectedItem(item)}
                className={`w-full text-left p-2 rounded-lg border-2 flex items-center gap-3 transition-colors ${ITEM_RARITY_COLORS[item.rarity]} ${selectedItem === item ? 'bg-gray-700/80 ring-2 ring-yellow-400' : 'bg-gray-900/50 hover:bg-gray-800/50'}`}
              >
                <div className="w-10 h-10 bg-gray-800 rounded flex items-center justify-center text-xl flex-shrink-0 overflow-hidden">
      ...
      {/* Right Column: Item Details (Now Sticky) */}
      <div className="md:col-span-1">
        <div className="sticky top-0">
            {selectedItem ? (
            <div className={`p-4 rounded-lg border-2 bg-gray-900/70 flex flex-col ${ITEM_RARITY_COLORS[selectedItem.rarity]}`}>
                {/* Header */}
                <div className="pb-3 border-b border-gray-700">
                <div className="flex items-start gap-4">
                    <div className="w-16 h-16 bg-gray-800 rounded-lg flex items-center justify-center text-4xl flex-shrink-0 overflow-hidden">
                        {selectedItem.icon.startsWith('http') ? <img src={selectedItem.icon} alt={selectedItem.name} className="w-full h-full object-cover" /> : selectedItem.icon}
                    </div>
                    <div className="flex-grow">
                        <h4 className={`text-2xl font-bold ${ITEM_RARITY_TEXT_COLORS[selectedItem.rarity]}`}>
                            {selectedItem.name}
                            {selectedItem.upgradeLevel && selectedItem.upgradeLevel > 0 && <span className="text-yellow-400 font-bold ml-2">+{selectedItem.upgradeLevel}</span>}
                        </h4>
                ...
                {selectedItemStack && selectedItemStack.count > 1 && (
                ...
                <div className="pt-3 border-t border-gray-700 flex flex-col gap-2">
                    ...
                    {(selectedItem.type === 'Tiêu hao' || selectedItem.type === 'Đan Dược') && <button onClick={() => onItemUse(selectedItem)} className="w-full bg-green-600 p-2 rounded hover:bg-green-500 font-semibold">Sử Dụng</button>}
