...
import { TITLE_ADVENTURES } from '../data/titleAdventures.ts';

interface AdventurePanelProps {
  onStartBattle: (monsterName: string) => void;
  onReceiveReward: (reward: { exp?: number; linhThach?: number; itemId?: string, rewardTitle?: string }) => void;
}
...
const AdventurePanel: React.FC<AdventurePanelProps> = ({ onStartBattle, onReceiveReward }) => {
  ...
  const startNewAdventure = useCallback(async () => {
    ...
    let newStorylet: AdventureStorylet | null = null;
    
    // MODIFIED: 30% chance to start a pre-written title adventure
    if (Math.random() < 0.3 && TITLE_ADVENTURES.length > 0) {
        newStorylet = TITLE_ADVENTURES[Math.floor(Math.random() * TITLE_ADVENTURES.length)];
    } else {
        newStorylet = await generateAdventureStorylet();
    }
