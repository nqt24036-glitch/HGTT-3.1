...
import { Item, Skill, ItemRarity, ItemType, EquipmentSlot, CombatStats, Monster, SpiritRootType } from '../types.ts';
...
// FIX: Added import for SPIRIT_ROOT_CLASSIFICATIONS to be used when updating a new monster's spirit root.
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose, onAddItem, onLearnSkill, onCreateItem, onCreateMonster, masterItemList }) => {
    ...
    const getInitialNewMonster = (): Monster => ({
        ...
        name: '',
// FIX: Add missing 'level' and 'cultivation' properties to satisfy the Monster type.
        level: 1,
        cultivation: { realm: 'Luyện Khí', stage: 1, lp: 0, lpToNext: 0 },
        hp: 100,
        stats: {
        ...
        skills: [],
        // FIX: Replaced deprecated 'linhCan' property with the 'spiritRoot' object to match the Monster type definition.
        spiritRoot: { classificationId: 'thien', attributes: ['Kim'], multiplier: 1.7 },
...
// FIX: Updated monster change handler to correctly update the 'spiritRoot' property instead of the deprecated 'linhCan'.
            } else if (field === 'spiritRoot') {
                const newAttribute = value as SpiritRootType;
                updatedMonster.spiritRoot.attributes = [newAttribute];
    
                // Automatically update classification based on attribute type for simplicity
                const isMutated = ['Phong', 'Lôi', 'Băng'].includes(newAttribute);
                const classificationId = isMutated ? 'bien_di' : 'thien';
                const classificationInfo = SPIRIT_ROOT_CLASSIFICATIONS.find(c => c.id === classificationId)!;
    
                updatedMonster.spiritRoot.classificationId = classificationInfo.id;
                updatedMonster.spiritRoot.multiplier = classificationInfo.expMultiplier;
            } else if (field === 'hp') {
        ...
        <div className="flex flex-col md:flex-row gap-6 h-full">
            <div className="flex flex-col h-full md:w-1/2 min-h-0">
                ...
                <div className="flex-grow space-y-2 overflow-y-auto pr-2 border-r-0 md:border-r border-gray-700">
                    {equippableItems.map(item => (<button key={item.id} onClick={() => setSelectedItem(item)} className={`w-full text-left p-2 rounded-lg border-2 flex items-center gap-3 transition-colors ${ITEM_RARITY_COLORS[item.rarity]} ${selectedItem?.id === item.id ? 'bg-gray-700/80 ring-2 ring-yellow-400' : 'bg-gray-900/50 hover:bg-gray-800/50'}`}><div className="w-10 h-10 bg-gray-800 rounded flex items-center justify-center text-xl flex-shrink-0 overflow-hidden">
    {item.icon.startsWith('http') ? <img src={item.icon} alt={item.name} className="w-full h-full object-cover" /> : item.icon}
</div><div className="flex-grow"><p className={`font-semibold ${ITEM_RARITY_TEXT_COLORS[item.rarity]}`}>{item.name}</p><p className="text-xs text-gray-400">{item.type} - {item.rarity}</p></div></button>))}
                </div>
            </div>
    ...
    const renderCreateMonsterTab = () => (
        <div className="flex flex-col md:flex-row gap-6 h-full">
            <div className="md:w-1/2 flex flex-col gap-3 overflow-y-auto pr-2">
                ...
                <label className="block"><span className="text-gray-400">Linh Căn</span>
                    {/* FIX: Updated monster creation UI to use 'spiritRoot' instead of the deprecated 'linhCan', resolving type errors. */}
                    <select value={newMonster.spiritRoot.attributes[0]} onChange={e => handleNewMonsterChange('spiritRoot', e.target.value as SpiritRootType)} className="w-full bg-gray-800 border border-gray-600 rounded px-2 py-1 mt-1">
                        {SPIRIT_ROOTS_DATA.map(sr => <option key={sr.id} value={sr.id}>{sr.name}</option>)}
                    </select>
                </label>
